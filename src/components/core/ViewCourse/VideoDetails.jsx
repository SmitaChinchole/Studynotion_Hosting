...
const VideoDetails = () => {
  ...
  const isFirstVideo = () => {
    const currentSectionIndex = courseSectionData?.findIndex(data => data._id === sectionId) ?? -1;
  ...
  const goToNextVideo = () => {
    ...
    if(currentSubSectionIndex !== noOfSubSections - 1) {
        // ✅ FIXED: Use currentSubSectionIndex not currentSectionIndex
    ...
    const handleLectureCompletion = async () => {
      ...
      try {
        ...
        if (res === true || res?.success) {
          ...
          // 🔥 YE 1 LINE ADD KARO (TOP pe import bhi karna)
          dispatch(getUserEnrolledCourses(token)); // Progress table refresh!
